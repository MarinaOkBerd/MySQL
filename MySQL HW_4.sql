...
SELECT s.*, Murzik_in_shop.*
FROM shops AS s
JOIN
(SELECT name, shops_id
FROM cats WHERE name = 'Murzik') AS Murzik_in_shop
ON s.id = Murzik_in_shop.shops_id;

SELECT s.*
FROM shops AS s
LEFT JOIN cats ON s.id = cats.shops_id
...
SELECT a.an_name AS 'Анализ', a.an_price 'Цена анализа'
FROM analysis AS a
JOIN orders AS o ON a.an_id = o.ord_an
WHERE o.ord_datetime >= '2020-02-05' AND  o.ord_datetime <= '2020-02-11';
